...
from src.windows.popup import Popup
...
class Hotkeys(Popup):
    def __init__(self, parent, main_app):
        ...
        Button(hotkeyLine, text="Clear", command=lambda: hotkeyManager.clear_hot_key("Record_Start", self.startKey)).grid(row=0, column=2,
                                                                                                                          padx=10)
        Button(hotkeyLine, text="Clear", command=lambda: hotkeyManager.clear_hot_key("Record_Stop", self.stopKey)).grid(row=1, column=2,
                                                                                                                        padx=10)
        Button(hotkeyLine, text="Clear", command=lambda: hotkeyManager.clear_hot_key("Playback_Start", self.playbackStartKey)).grid(row=2,
                                                                                                                                    column=2,
                                                                                                                                    padx=10)

        Button(hotkeyLine, text="Start Record",
               command=lambda: hotkeyManager.enable_hot_key_detection("Record_Start", self.startKey, 0)).grid(
            row=0,
            column=0,
            padx=10)
        ...
        Button(hotkeyLine, text="Stop Record", command=lambda: hotkeyManager.enable_hot_key_detection("Record_Stop", self.stopKey, 1)).grid(
            row=1,
            column=0,
            padx=10)
